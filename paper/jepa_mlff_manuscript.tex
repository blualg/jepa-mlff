...
We distinguish the coordinate trajectory used for self-supervised pretraining from the labelled data used for supervised force-field learning:
\begin{equation}
    \mathcal{D}_{\mathrm{coord}}
    =
    \{(Z,R_t)\}_{t=1}^{T},
    \qquad
    \mathcal{D}_{\mathrm{lab}}
    =
    \{(Z,R_t,E_t,F_t)\}_{t=1}^{T_{\mathrm{lab}}},
\end{equation}
where $Z \in \mathbb{N}^{N}$ are atomic numbers for $N$ atoms, $R_t \in \mathbb{R}^{N \times 3}$ are Cartesian coordinates, $E_t \in \mathbb{R}$ is the reference potential energy and $F_t \in \mathbb{R}^{N \times 3}$ are reference forces. JEPA-style pretraining uses only $\mathcal{D}_{\mathrm{coord}}$; energies and forces are introduced only during supervised fine-tuning. The encoder $f_{\theta}$ maps atomic numbers and coordinates to atomwise latent embeddings,
\begin{equation}
    H_t = f_{\theta}(Z, R_t) \in \mathbb{R}^{N \times d}.
\end{equation}

For JEPA-style pretraining, we sample a binary atom mask $m \in \{0,1\}^{N}$, where $m_i=1$ indicates that atom $i$ is masked in the context branch and the implementation guarantees $\sum_i m_i \ge 1$. Masking is applied to the initial atom embeddings rather than to Cartesian coordinates:
\begin{equation}
    e_i^{\mathrm{ctx}}
    =
    \begin{cases}
    e(Z_i), & m_i = 0,\\
    e_{\mathrm{mask}}, & m_i = 1,
    \end{cases}
\end{equation}
where $e_{\mathrm{mask}}$ is a learned mask token. Thus, the coordinates $R_t$ and their pairwise distances remain visible to the context encoder, while the species/atom embedding of each masked atom is hidden. The context and target representations are
\begin{equation}
    H_t^{\mathrm{ctx}} = f_{\theta}(Z,R_t;m),
    \qquad
    \bar{H}_{t+\Delta} = f_{\bar{\theta}}(Z,R_{t+\Delta}).
\end{equation}
With online encoder $f_{\theta}$, target encoder $f_{\bar{\theta}}$, predictor $q_{\phi}$ and stop-gradient operator $\operatorname{sg}(\cdot)$, the pretraining loss is
\begin{equation}
...
m_i
\left\|
q_{\phi}\!\left(H_{t,i}^{\mathrm{ctx}}\right)
-
\operatorname{sg}\!\left(\bar{H}_{t+\Delta,i}\right)
\right\|_2^2 .
\label{eq:jepa}
\end{equation}
...
\begin{equation}
    \hat{F}_{\theta,\psi}(Z,R)
    =
    -\nabla_R \hat{E}_{\theta,\psi}(Z,R),
    \qquad
    \hat{F}_{i\alpha}
    =
    -\frac{\partial \hat{E}_{\theta,\psi}}{\partial R_{i\alpha}}.
    \label{eq:forces}
\end{equation}
...
\caption{\textbf{Training dynamics and test performance.} (a) JEPA-style pretraining loss for masked latent prediction. The validation loss reaches its minimum at epoch 9. (b) Validation force RMSE during supervised fine-tuning for scratch and JEPA-style-initialized models. (c) Final test metrics on the held-out MD17 ethanol split. (d) Relative improvement of JEPA-style initialization over scratch training.}
\label{fig:summary}
\end{figure}
...
\caption{\textbf{Force prediction scatter for scratch and JEPA-style-initialized models.} Predicted force components are plotted against DFT reference force components for the held-out MD17 ethanol test split. The diagonal line indicates perfect agreement. The JEPA-style-initialized model exhibits a lower force RMSE than the scratch baseline after the same number of supervised epochs.}
...
\subsection*{JEPA-style pretraining}
The JEPA-style stage uses an online encoder, an EMA target encoder and a predictor MLP. For each molecular frame, a random subset of initial atom embeddings is masked in the online branch while Cartesian coordinates remain visible. The target branch receives the uncorrupted frame at temporal offset $\Delta=1$. The training loss is masked latent mean-squared error between predictor outputs and stop-gradient target embeddings. The default mask ratio is 0.35 and EMA decay is 0.99. The 50-epoch pretraining checkpoint is \path{checkpoints/jepa_pretrained_50.pt}.

\subsection*{Supervised fine-tuning}
Scratch and JEPA-style-initialized force fields were trained with the same supervised objective in Eq.~\ref{eq:supervised}. Both models were trained for 50 epochs using AdamW. The scratch model was initialized randomly. The JEPA-style model loaded the pretrained encoder weights and initialized a fresh energy head.
